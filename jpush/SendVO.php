...
class SendVO
{
    ...
    public function __construct($initparams, $params, $extras)
    {
        ...
        if ($params["mes_type"] == 1)
        {
            ...
            $mes_title = $params["title"];
            $mes_content = $params["content"];
            if ($build_id == 0) {
                $content = array('n_title'=>$mes_title, 'n_content'=>$mes_content, 'n_extras'=>$extras);
            } else {
                 $content = array('n_title'=>$mes_title, 'n_content'=>$mes_content, 'n_extras'=>$extras, 'n_builder_id'=>$build_id);
            }
            
        }
        else if($params["mes_type"] == 2)
        {
            $mes_content_type = $params["content_type"];
            $mes_title = $params["title"];
            $mes_content = $params["content"];
            if ($mes_content_type == '') {
                $content = array('title'=>$mes_title, 'message'=>$mes_content, 'extras'=>$extras);
            } else {
                $content = array('title'=>$mes_title, 'message'=>$mes_content, 'extras'=>$extras, 'content_type'=>$mes_content_type);
            }
